...
export class SubSubCatgeoryService {
  ...
  updateSubSubCategory(subSubCategoryId: number, subsubcategory: Subsubcategory): Observable<any> {
    const url = `${this.baseURL}/updateSubSubCategory/${subSubCategoryId}/${subsubcategory.categoryid}/${subsubcategory.subcategoryId}`;
  ...
  getSubSubCategoriesBySubcategoryId(subcategoryId: number): Observable<Subsubcategory[]> {
    const url = `${this.baseURL}/subcategories/${subcategoryId}/subsubcategories/`;
    console.log('API URL:', url); // Log the URL to verify it is correct
    return this.httpClient.get<Subsubcategory[]>(url);
  }
}
